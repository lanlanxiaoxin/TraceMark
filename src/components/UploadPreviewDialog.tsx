...
export function UploadPreviewDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" role="dialog" aria-modal="true" aria-labelledby="upload-preview-title">
      <div className="w-full max-w-lg max-h-[85vh] flex flex-col rounded-2xl bg-white shadow-xl border border-gray-200">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          ...
          {preview.payloadSummary ? (
            <div>
              <p className="text-xs font-medium text-gray-500 mb-2">将发送的内容摘要</p>
        ...
        <div className="flex gap-2 px-5 py-4 border-t border-gray-100">
          <button type="button" onClick={onCancel} disabled={busy} className="flex-1 py-2 text-sm rounded-lg border border-gray-200 text-gray-700">取消</button>
          <button type="button" onClick={onConfirm} disabled={busy || !preview.canProceed} className="flex-1 py-2 text-sm font-medium rounded-lg bg-gray-900 text-white disabled:opacity-40">{busy ? '生成中…' : confirmLabel}</button>
        </div>
      </div>
    </div>
  )
