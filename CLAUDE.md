...
公开仓库以本文件与 [README.md](README.md) 为准；完整 PRO 方案文档（`docs/`）仅保留在本地，不随 GitHub 发布。
...
- PRO5.0 的方向是长期资产沉淀，但每次开发仍应落到最小可验证增量。
...
- 主入口优先围绕“今日资产收件箱”，但 Timeline / 报告仍作为底部并列入口保留，便于排障与历史回看。
- 复盘生成必须优先使用 confirmed assets，而不是直接基于原始 activity logs。
- 项目级能力优先进入 `project_spaces` / `project_aliases`，不要继续堆进全局 `settings`。
- 增强摘要必须分级授权；没有授权时只能降级，不得绕过。
- 本地轻量模型只做预留或实验，不应阻塞 W1-W6 主线。

---

## 方案文档（仅本地）

详细 PRD（PRO3–PRO5）位于仓库根目录 `docs/`，**未纳入公开 Git 仓库**。协作与 AI 编码请以本文件、`README.md` / `README.zh-CN.md` 及源码为准。
...
- **今日** 为默认入口，承载工作资产收件箱（PRO5.0 主流程）
- **项目** 管理项目空间与已确认资产
- **时间轴** 保留 PRO3.0/4.0 的原始活动查看能力（排障、回看）
- **报告** 保留 PRO4.0 的日报 / 周报生成（辅助导出）
- **设置** 监听、隐私、AI、分类、通知

资产库（W5）与复盘（W6）以「今日」/「项目」内嵌或子页面落地，不再单独占用底部主导航位置。
...
| 多平台抽象 | ActivityProvider | PRO5.0 要新增的平台采集适配层 |
...
| 周期 | 焦点 | 交付 |
|------|------|------|
| W1 | 工程基线 | 修复真实 typecheck/build，统一包管理器 |
| W2 | 项目空间与数据模型 | `project_spaces`、`project_aliases`、`work_assets`、`privacy_consents`、`retrospectives` |
| W3 | 候选资产生成 | 从 activity_logs + Git + 标题摘要生成 3-8 张候选卡 |
| W4 | 今日资产收件箱 | 确认、编辑、忽略、合并、私密标记、补充一句话 |
| W5 | 资产库与项目知识库 | 列表、过滤、证据链、Markdown 导出 |
| W6 | 复盘生成 | 周复盘、项目阶段复盘、只使用 confirmed 资产 |
| W7 | 隐私与增强摘要 | 上传预览、分级授权、文档片段、会议/聊天标题摘要 |
| W8 | 多平台抽象与内测 | ActivityProvider、Windows 完整闭环、macOS PoC |
...
1. 修复 `npm run typecheck` 误报。
2. 修复主进程和渲染进程 TypeScript 错误。
3. 统一包管理器。
4. 确认 `npm run build` 可通过。

工程基线稳定后，再进入项目空间和工作资产卡片开发。
